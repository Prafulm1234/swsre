...
`beforeHook` can mutate `options` before itâs passed to `fetchFromDatabase`.
...
That way you donât need to expose the [hookCollection()](#hookcollection) method to consumers of your library
...
<table>
  ...
  <tr>
    ...
    <td>Function</td>
    <td>
      Executed before the wrapped method. Called with the hookâs
...
<table>
  ...
  <tr>
    ...
    <td>
      ...
      and the hookâs <code>options</code> argument. The first <code>method</code>
...
<table>
  ...
  <tr>
    ...
    resolves with the hookâs <code>options</code> argument.
    </td>
    <td>Yes</td>
  </tr>
</table>
