...
//     console.error('f.length: %d     (should be 12)', f.length);
...

  {
    ...
    var size = f.write('ããããã', encoding);
...
['ucs2', 'ucs-2', 'utf16le', 'utf-16le'].forEach(function(encoding) {
  ...
  b.write('ããããã', encoding);
  assert.equal(b.toString(encoding), 'ããããã');
});
...

{
  ...
  written = buf.write('ã'); // 3bytes
  assert.equal(written, 0);
  written = buf.write('\0ã'); // 1byte + 3bytes
  assert.equal(written, 1);
  written = buf.write('\0\0ã'); // 1byte * 2 + 3bytes
  assert.equal(written, 2);
}

{
  var buf = Buffer.allocUnsafe(10);
  written = buf.write('ããã'); // 3bytes * 3 (v8 adds null terminator)
  assert.equal(written, 9);
  written = buf.write('ããã\0'); // 3bytes * 3 + 1byte
