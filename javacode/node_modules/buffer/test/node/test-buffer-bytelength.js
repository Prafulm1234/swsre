...
// utf8
assert.equal(Buffer.byteLength('âÃ©llÃ¶ wÃ¸rlâ!', 'utf-8'), 19);
assert.equal(Buffer.byteLength('ÎºÎ»Î¼Î½Î¾Î¿', 'utf8'), 12);
assert.equal(Buffer.byteLength('æµæ¶æ·æ¸æ¹', 'utf-8'), 15);
assert.equal(Buffer.byteLength('ð ¹ð ±ð ±¸', 'UTF8'), 12);
// without an encoding, utf8 should be assumed
assert.equal(Buffer.byteLength('hey there'), 9);
assert.equal(Buffer.byteLength('ð ±¸æ¶Î½Î¾#xx :)'), 17);
assert.equal(Buffer.byteLength('hello world', ''), 11);
// it should also be assumed with unrecognized encoding
assert.equal(Buffer.byteLength('hello world', 'abc'), 11);
assert.equal(Buffer.byteLength('ÃÅââ', 'unkn0wn enc0ding'), 10);
...
assert.equal(Buffer.byteLength('Il Ã©tait tuÃ©'), 14);
assert.equal(Buffer.byteLength('Il Ã©tait tuÃ©', 'utf8'), 14);
assert.equal(Buffer.byteLength('Il Ã©tait tuÃ©', 'ascii'), 12);
assert.equal(Buffer.byteLength('Il Ã©tait tuÃ©', 'latin1'), 12);
assert.equal(Buffer.byteLength('Il Ã©tait tuÃ©', 'binary'), 12);
['ucs2', 'ucs-2', 'utf16le', 'utf-16le'].forEach(function(encoding) {
  assert.equal(24, Buffer.byteLength('Il Ã©tait tuÃ©', encoding));
});
