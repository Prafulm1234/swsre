...
module.exports = class extends BaseGenerator {
  ...
  get prompting() {
    return {
      ...
      createEKSCluster() {
        ...
        const done = this.async();
        this.log.ok('Initialising Elastic Kubernetes Service (EKS)ï¸. This can take up to 15 minutes depending on load....');
        shelljs.exec(
  ...
  get configuring() {
    return {
      bonjour() {
        if (this.abort) return;
        this.log(chalk.bold('ð§ð ï¸ AWS configuring'));
      },
      purgeAwsApps() {
        if (this.abort) return;
  ...
  get end() {
    return {
      ...
      loginToAws() {
        ...
        const done = this.async;
        return dockerCli
          .loginToAws(this.aws.region, this.aws.dockerLogin.accountId, this.aws.dockerLogin.username, this.aws.dockerLogin.password)
          .then(() => {
