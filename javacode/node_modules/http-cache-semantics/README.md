...
-   `policy` â A new `CachePolicy` with HTTP headers updated from `revalidationResponse`. You can always replace the old cached `CachePolicy` with the new one.
-   `modified` â Boolean indicating whether the response body has changed.
